...
/**
 * LLM-Enhanced Model Context Protocol Implementation
 *
...
class KnowledgeBaseManager {
    ...
    async syncLatestSpecifications() {
        ...
        const fetchAndSaveContent = async (url, filePath) => {
            try {
        ...
        try {
            ...
            await fetchAndSaveContent(`${SPECIFICATION_REPO_URL}/raw/main/schema/schema.json`, path.join(specDir, 'schema.json'));
            // Fetch example files
            await fetchAndSaveContent(`${SPECIFICATION_REPO_URL}/raw/main/schema/examples/resource-response.json`, path.join(examplesDir, 'resource-response.json'));
            // Always write version information
            await fs.writeFile(path.join(specDir, 'version.json'), JSON.stringify({ version: LATEST_PROTOCOL_VERSION, lastUpdated: new Date().toISOString() }));
    ...
    async getLatestProtocolVersion() {
        try {
            const versionFile = await fs.readFile(path.join(this.basePath, 'specifications', 'version.json'), 'utf-8');
    ...
    async getSpecification(version) {
        ...
        try {
            ...
            console.error('Using fallback specification');
            return null;
        }
    }
}
...
class McpHost {
    ...
    async createServer(name) {
        ...
        server.resource("mcp-docs", new ResourceTemplate("mcp-docs://{topic}", { list: undefined }), async (uri, { topic }) => {
...
export { KnowledgeBaseManager, McpHost, };
